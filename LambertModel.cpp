...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* LambertModel::device = nullptr;
ID3D12GraphicsCommandList* LambertModel::sCommandList = nullptr;
LambertModel::PipelineSet LambertModel::pipelineSet;
Camera* LambertModel::sCamera_ = nullptr;

void LambertModel::StaticInitialize(ID3D12Device* device, Camera* camera)
{
	// nullptrチェック
	assert(device);

	LambertModel::device = device;
	sCamera_ = camera;
...
void LambertModel::CreateGraphicsPipeline()
{
...
void LambertModel::PreDraw(ID3D12GraphicsCommandList* commandList)
{
...
void LambertModel::PostDraw()
{
	// コマンドリストを解除
	sCommandList = nullptr;
}

LambertModel* LambertModel::Create()
{
	// 3Dオブジェクトのインスタンスを生成
	LambertModel* lambertModel = new LambertModel();
	if (lambertModel == nullptr) {
		return nullptr;
	}

	// 初期化
	if (!lambertModel->Initialize()) {
		delete lambertModel;
		assert(0);
		return nullptr;
	}
