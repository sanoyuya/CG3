...
/// 静的メンバ変数の実体
/// </summary>
ID3D12Device* BaseModel::device = nullptr;
ID3D12GraphicsCommandList* BaseModel::sCommandList = nullptr;
BaseModel::PipelineSet BaseModel::pipelineSet;
Camera* BaseModel::sCamera_ = nullptr;

void BaseModel::StaticInitialize(ID3D12Device* device, Camera* camera)
{
	// nullptrチェック
	assert(device);

	BaseModel::device = device;
	sCamera_ = camera;
...
void BaseModel::CreateGraphicsPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Resources/shaders/BaseVS.hlsl", // シェーダファイル名
		nullptr,
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources/shaders/BasePS.hlsl", // シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
void BaseModel::PreDraw(ID3D12GraphicsCommandList* commandList)
{
